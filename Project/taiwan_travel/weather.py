...
def weather_crawler():
    ...
    for dataset_id in FD:
        url = f"https://opendata.cwa.gov.tw/api/v1/rest/datastore/{dataset_id}?Authorization={authorization}&format=JSON&elementName=WeatherDescription"
        # 發送請求並解析返回的 JSON 資料
        ...
        for i in range(num_locations):
            ...
            Px = DA["records"]["locations"][0]["location"][i]["lon"]
            Py = DA["records"]["locations"][0]["location"][i]["lat"]
            timeDT = pd.DataFrame()
            # 獲取每個鄉鎮中時間的數量
            num_times = len(DA["records"]["locations"][0]
                            ["location"][i]["weatherElement"][0]["time"])
            # 遍歷每個時間點
            for j in range(num_times):
                startTime = DA["records"]["locations"][0]["location"][i]["weatherElement"][0]["time"][j]["startTime"]
                endTime = DA["records"]["locations"][0]["location"][i]["weatherElement"][0]["time"][j]["endTime"]
                value = DA["records"]["locations"][0]["location"][i]["weatherElement"][0]["time"][j]["elementValue"][0]["value"]
                # 將數據組成單行 DataFrame
                data = pd.DataFrame({
                    "Region": [Region],
                    "Town": [Town],
    ...
    AllData['降雨機率'] = AllData['降雨機率'].str.extract(r'(\d+\.?\d*)', expand=False).astype(float)
    AllData['預報溫度'] = AllData['預報溫度'].str.extract(r'(\d+\.?\d*)', expand=False).astype(float)
    AllData['相對溼度'] = AllData['相對溼度'].str.extract(r'(\d+\.?\d*)', expand=False).astype(float)
